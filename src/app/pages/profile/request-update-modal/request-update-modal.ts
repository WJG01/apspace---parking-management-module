...
export class RequestChangeModalPage implements OnInit {
  ...
  submit() {
    if (this.orientationProfile.councelor_details.length < 1) {
      this.component.toastMessage('No E-Counsellor Has Been Assigned To You Yet! Please Contact The Admin Office.', 'danger');
    } else {
      ...
      if (Object.keys(body).length <= 3) {
        ...
        this.component.toastMessage('Nothing Has Been Changed In The Form, Request Cannot Be Submitted Without Any Changes.', 'danger');
      } else {
        this.presentLoading();
        ...
        this.ws.post('/orientation/profile_change_request', { body }).subscribe(
          ...
          () => {
            this.dismissLoading();
            this.component.toastMessage('Your Request Change Has Been Submitted Successfully. The Team Will Review Your Request Now.', 'success');
            this.dismiss();
          }
        );
      }
